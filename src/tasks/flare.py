...
class MT(Task):
    ...
    DATASET_NAME = None
    EVAL_LAST_TURN = True
    DATASET_PATH = "/data/chenzhengyu/datasets/MT/tense"
    ...
    def process_results(self, doc, results):
        return {
            "bleu": (doc["output"], results[0]),
            "chrf": (doc["output"], results[0]),
            "ter": (doc["output"], results[0]),
            "comet": (doc["output"], results[0]),
        }
...
"""
    ...
    def comet(items):
        ...
        for num in range(len(refs)):
            data.append({"src": refs[num],"mt": preds[num]})
    ...
    def chrf_score(self, items):
        results = chrf(items)
        return results

    def ter_score(self, items):
        results = ter(items)
        return results

    def comet_score(self, items):
        results = MT.comet(items)
    ...
    def aggregation(self):
        return {
            ...
            "ter": self.ter_score,
            "comet": self.comet_score,
        }

class MT_idiom(MT):
    DATASET_PATH = "ChanceFocus/mt-idiom"

class MT_stencepair(MT):
    DATASET_PATH = "ChanceFocus/mt-stencepair"

class MT_tense(MT):
    DATASET_PATH = "ChanceFocus/mt-tense"

class MT_zpt(MT):
    DATASET_PATH = "ChanceFocus/mt-zpt"
"""
